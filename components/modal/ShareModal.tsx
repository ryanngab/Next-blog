...
const cleanUrl = (url: string) => {
  // Menghapus karakter yang tidak diinginkan dari URL
  return url.replace(/[%#&?]/g, ''); // Contoh: menghapus %, #, &, ?
};

export function ShareModal() {
  const rawShareLink = window.location.href; // Mengambil URL saat ini
  const shareLink = cleanUrl(rawShareLink); // Membersihkan URL
  const articleTitle = document.title; // Mengambil judul artikel
    ...
    switch (platform) {
      ...
      case 'pinterest':
        url = `https://pinterest.com/pin/create/button/?url=${encodeURIComponent(
          shareLink
        )}&description=${encodeURIComponent(articleTitle)}`;
        break;
      case 'linkedin':
      ...
      case 'email':
        ...
        break;
      case 'bluesky':
        url = `https://bsky.app/share?link=${encodeURIComponent(
          shareLink
        )}&text=${encodeURIComponent(articleTitle)}`;
        break;
      case 'threads':
        url = `https://www.threads.net/share?link=${encodeURIComponent(
          shareLink
        )}&text=${encodeURIComponent(articleTitle)}`;
        break;
      default:
