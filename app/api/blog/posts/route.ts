...
export async function GET(req: Request) {
  ...
  const pageSize = parseInt(urlParams.searchParams.get('pageSize') || '6');
  ...
  try {
    do {
      const url = `https://www.googleapis.com/blogger/v3/blogs/${blogId}/posts?key=${apiKey}&maxResults=${maxResults}&publishedAfter=2024-01-01T00:00:00Z${nextPageToken ? `&pageToken=${nextPageToken}` : ''}`;
